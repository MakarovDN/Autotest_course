...
def treatment_sum(our_tuple):
    """
    Функция получает входящий кортеж и проверяет:
    - Если в кортеже 2 элемента и их можно сложить, то возвращаем результат;
    - Если в кортеже 2 элемента и их нельзя сложить, то обрабатываем исключение и возвращаем 'Нельзя сложить эти данные';
    - Если в кортеже меньше двух элементов, то обрабатываем исключение и возвращает 'Недостаточно данных';
    - Если в кортеже больше двух элементов, то генерируем исключение Exception с текстом 'Много данных'.
    :param our_tuple: входящий кортеж
    :return: результат сложения или исключение
    """
    if len(our_tuple) > 2:
